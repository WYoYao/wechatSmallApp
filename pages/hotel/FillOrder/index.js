...
Page({
  ...
  _GetUserInfoByJSJID(cb) {
    ...
    new Jrequest("JUser").get("GetUserInfoByJSJID", {
      ...
      if (data.BaseResponse.IsSuccess) {
        ...
        this.setData({
          state: Object.assign(this.data.state, {
            CardTypeId: data.CardList[0].CardTypeID,
            Contact: {
  ...
  _GetOrderGuranteeInfoNew() {
    //调用酒店担保信息
    new Jrequest("HotelApi").get("_GetOrderGuranteeInfoNew", this.data._GetOrderGuranteeInfoNew, data => {
